...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse): Promise<void> {
  try {
    ...
    const result = await printifyService.bulkEnableProducts(product_ids, userId, reason)
    ...
    res.status(500).json({ success: false, error: "Internal server error", message: "Failed to bulk enable products" })
  }
}
