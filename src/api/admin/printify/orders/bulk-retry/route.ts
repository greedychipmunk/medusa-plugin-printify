...
export async function POST(req: AuthenticatedMedusaRequest, res: MedusaResponse): Promise<void> {
  try {
    ...
    const bodyValidation = bulkRetrySchema.safeParse(req.body)
    ...
    const result = await printifyService.bulkRetryOrders(order_ids)

    res.status(200).json({
      success: true,
      message: `Bulk retry completed: ${result.success_count} successful, ${result.failure_count} failed`,
      data: {
    ...
    apiLogger.error("Failed to bulk retry orders", error as Error)
    ...
    res.status(500).json({ success: false, error: "Internal server error", message: "Failed to bulk retry orders" })
  }
}
